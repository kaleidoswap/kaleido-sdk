...
class KaleidoClient:
    ...
    def _map_error(self, e: Exception) -> Exception:
        ...
        if "401" in msg or "403" in msg or "Unauthorized" in msg or "Forbidden" in msg or "Authentication failed" in msg:
        ...
        if "Insufficient balance" in msg or "Insufficient capacity" in msg or "not enough" in msg:
        ...
        if "Validation" in msg or "valid invoice" in msg or "Invalid params" in msg or "Type validation" in msg:
    ...
    def market(self) -> MarketClient:
        ...
        if self._market_client is None:
            self._market_client = MarketClient(self._inner, self._parse_response, self._execute)
    ...
    def orders(self) -> OrdersClient:
        ...
        if self._orders_client is None:
            self._orders_client = OrdersClient(self._inner, self._parse_response, self._execute)
    ...
    def swaps(self) -> SwapsClient:
        ...
        if self._swaps_client is None:
            self._swaps_client = SwapsClient(self._inner, self._parse_response, self._execute)
    ...
    def lsp(self) -> LspClient:
        ...
        if self._lsp_client is None:
            self._lsp_client = LspClient(self._inner, self._parse_response, self._execute)
    ...
    def node(self) -> Optional[NodeClient]:
        ...
        if self._node_client is None:
            self._node_client = NodeClient(self._inner, self._parse_response, self._execute)
    ...
    def get_quote_by_pair(
        ...
        """
        json_str = self._execute(self._inner.get_quote_by_pair, ticker, from_amount, to_amount)
    ...
    def get_best_quote(
        ...
        Returns:
            PairQuoteResponse object
        """
        json_str = self._execute(self._inner.get_best_quote, ticker, from_amount, to_amount)
        return self._parse_response(json_str, PairQuoteResponse)
    ...
    def execute_swap_flow(
        ...
        """
        from .generated_models import SwapRequest, ConfirmSwapRequest, SwapResponse
    ...
    def execute_swap_by_pair(
        ...
        if len(parts) != 2:
            raise ValueError(f"Invalid pair ticker: {pair_ticker}. Expected format: BASE/QUOTE")
        
        base_ticker, quote_ticker = parts
        
        # Get assets
        base_asset = self.get_asset_by_ticker(base_ticker)
        quote_asset = self.get_asset_by_ticker(quote_ticker)
        ...
        if side_lower == "sell":
            ...
            quote = self.get_quote_by_pair(pair_ticker, from_amount=raw_amount, to_amount=None)
        elif side_lower == "buy":
            # Buying base asset: to_amount is what we want to receive
            raw_amount = self.to_raw(amount, base_asset)
            quote = self.get_quote_by_pair(pair_ticker, from_amount=None, to_amount=raw_amount)
        else:
