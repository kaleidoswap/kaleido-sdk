...
class TestBtcLnToRgbSwap:
    ...
    def test_create_swap_order_btc_ln_to_rgb_onchain(self, client):
        ...
        if len(pairs) == 0:
            pytest.skip("No trading pairs available")
        ...
        if target_pair is None:
            pytest.skip("Could not find a BTC/RGB trading pair, skipping test")
        ...
        btc_endpoint = next(
            (e for e in btc_asset.endpoints if e.layer == "BTC_LN"), None
        )
        ...
        print(
            f"Using pair {pair_ticker}. Requesting from_amount={from_amount} BTC to acquire {rgb_asset.ticker}"
        )
        ...
        try:
            ...
            if isinstance(e, ResourceNotFoundError) or "not found" in str(e).lower():
                pytest.skip(f"Quote not found for {pair_ticker}: {e}")
        ...
        request = CreateSwapOrderRequest(
            ...
            receiver_address=ReceiverAddress(
                address=real_addr, format=ReceiverAddressFormat.btc_address
            ),
        )
        ...
        print(
            f"Creating swap order with RFQ {quote.rfq_id} using address {real_addr}..."
        )
