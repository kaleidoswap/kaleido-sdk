...
describe('Utility Functions', () => {
  ...
  describe('toSmallestUnits', () => {
    ...
    it('should handle different precision values', () => {
      expect(toSmallestUnits(1, 0)).toBe(1);
      expect(toSmallestUnits(1, 2)).toBe(100);
      expect(toSmallestUnits(1, 18)).toBe(1_000_000_000_000_000_000);
    });
  });

  describe('toDisplayUnits', () => {
    it('should convert satoshis to BTC (8 decimals)', () => {
      const result = toDisplayUnits(100_000_000, 8);
      expect(result).toBe(1.0);
    });

    it('should convert smallest units to USDT (6 decimals)', () => {
      const result = toDisplayUnits(1_000_000, 6);
      expect(result).toBe(1.0);
    });

    it('should handle fractional results', () => {
      const result = toDisplayUnits(50_000_000, 8);
      expect(result).toBe(0.5);
    });

    it('should handle very small amounts', () => {
      const result = toDisplayUnits(1, 8);
      expect(result).toBe(0.00000001);
    });

    it('should handle zero', () => {
      const result = toDisplayUnits(0, 8);
      expect(result).toBe(0);
    });

    it('should handle large amounts', () => {
      const result = toDisplayUnits(2_100_000_000_000_000, 8);
