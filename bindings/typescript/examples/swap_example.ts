 ...
 * Format amount with correct units based on layer
 */
function formatAmount(amount: string | number | bigint, ticker: string, layer: string): string {
  const amtNum = typeof amount === 'bigint' ? Number(amount) : Number(amount);
 ...
 */
function recoverPubkeyFromError(error: unknown): string | null {
...
function findBtcUsdtPair(pairs: any[]): any | null {
  for (const pair of pairs) {
    ...
    if (
      (fromTicker.includes('BTC') && toTicker.includes('USDT')) ||
...
async function main() {
  ...
  try {
    ...
    console.log(
      `      ${quote.from_asset.amount} ${quote.from_asset.ticker} → ` +
  ...
  try {
    ...
    const info =
      typeof nodeInfo === 'string' ? JSON.parse(nodeInfo) : (nodeInfo as { pubkey?: string });
  ...
  try {
    ...
    await client.maker.executeSwap(confirmRequest);
