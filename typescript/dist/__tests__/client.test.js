...
describe('KaleidoClient', () => {
    const client = (0, testUtils_1.createTestClient)();
    ...
    describe('trading pair operations', () => {
        ...
        it('should get pair by assets', async () => {
            try {
                const assetPair = await (0, testUtils_1.getPairAssetIds)(client);
                const foundPair = await client.getPairByAssets(assetPair.baseAssetId, assetPair.quoteAssetId);
    ...
    describe('quote operations', () => {
        it('should get quote', async () => {
            try {
                const assetsResponse = await (0, testUtils_1.assetListResponse)(client);
                const result = await client.getQuote(assetsResponse.fromAsset, assetsResponse.toAsset, assetsResponse.fromAmount);
                expect(result).toBeDefined();
    ...
    describe('WebSocket operations', () => {
        it('should get quote via websocket', async () => {
            try {
                const assetsResponse = await (0, testUtils_1.assetListResponse)(client);
                const result = await client.getQuoteWS(assetsResponse.fromAsset, assetsResponse.toAsset, assetsResponse.fromAmount);
                expect(result).toBeDefined();
                expect(result).toHaveProperty('rfq_id');
                expect(result).toHaveProperty('to_amount');
            }
    ...
    describe('swap operations', () => {
        it('should initialize maker swap', async () => {
            ...
            const init_result = await client.initMakerSwap(quote.rfq_id, quote.from_asset, quote.to_asset, quote.from_amount, quote.to_amount);
        ...
        it('should whitelist a trade', async () => {
            const assetsResponse = await (0, testUtils_1.assetListResponse)(client);
            const quote = await client.getQuoteWS(assetsResponse.fromAsset, assetsResponse.toAsset, 100000000);
            const initResult = await client.initMakerSwap(quote.rfq_id, quote.from_asset, quote.to_asset, quote.from_amount, quote.to_amount);
        ...
        it('should confirm complete swap', async () => {
            try {
                ...
                const initResult = await (0, testUtils_1.testWhiteListTrade)(client, 5, 2000); // 5 retries, 2s delay
                if (!initResult) {
