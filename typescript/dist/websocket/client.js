...
class WebSocketClient {
    ...
    async connect() {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                if (!this.ws) {
                    const error = new exceptions_1.WebSocketError('WebSocket instance is null');
                    debug(error.message);
                    reject(error);
                    return;
                }
                const timeout = setTimeout(() => {
                    const error = new exceptions_1.WebSocketError(`Connection timeout after ${this.pingTimeout}ms`);
                    debug(error.message);
                    reject(error);
                }, this.pingTimeout);
                this.ws.onopen = () => {
                ...
                this.ws.onerror = (event) => {
                    ...
                    const error = new exceptions_1.WebSocketError(`Connection error: ${errorMessage}`);
    ...
    attemptReconnect() {
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
            const error = new exceptions_1.WebSocketError(`Max reconnection attempts (${this.maxReconnectAttempts}) reached`);
            debug(error.message);
            this.emit('error', { action: 'max_reconnect_attempts', error: error.message });
            return;
        }
        const delay = Math.min(this.reconnectInterval * Math.pow(2, this.reconnectAttempts - 1), 30000 // Max 30 seconds between retries
        );
    ...
    async send(message) {
        ...
        if (!this.ws) {
            const error = new exceptions_1.WebSocketError('WebSocket is not connected');
        ...
        try {
            ...
            throw new exceptions_1.WebSocketError(`Failed to send message: ${errorMessage}`);
        }
    }
    /**
