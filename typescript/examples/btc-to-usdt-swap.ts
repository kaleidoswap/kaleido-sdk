/**
 * BTC to USDT Swap Example (Selling BTC for USDT)
 * 
...
async function btcToUsdtSwap() {
  ...
  try {
    ...
    // Step 4: Find BTC and USDT assets
    console.log('\nFinding BTC and USDT assets...');
    const btc = assetMapper.findByTicker('BTC');
    const usdt = assetMapper.findByTicker('USDT');

    if (!btc || !usdt) {
      throw new Error('BTC or USDT not found');
    }

    console.log(`Found: ${btc.ticker} (${btc.asset_id}) and ${usdt.ticker} (${usdt.asset_id})`);

    // Step 5: Check if trading pair exists
    if (!assetMapper.canTrade(btc.asset_id, usdt.asset_id)) {
      throw new Error('No trading pair between BTC and USDT');
    }

    // Step 6: Validate and prepare amount
    const decimalAmount = 0.00001; // 0.001 BTC
    console.log(`\nPreparing to swap ${decimalAmount} ${btc.ticker}...`);
    ...
    console.log(`- Decimal amount: ${decimalAmount} ${btc.ticker}`);
    ...
    const fromAmountDecimal = precisionHandler.toDecimalAmount(quote.from_amount, btc.asset_id);
    const toAmountDecimal = precisionHandler.toDecimalAmount(quote.to_amount, usdt.asset_id);
    const feeDecimal = quote.fee && typeof quote.fee === 'object' && 'final_fee' in quote.fee
      ? precisionHandler.toDecimalAmount((quote.fee as any).final_fee, usdt.asset_id)
      : 0;

    console.log(`Quote received:`);
    console.log(`- From: ${fromAmountDecimal} ${btc.ticker}`);
    console.log(`- To: ${toAmountDecimal} ${usdt.ticker}`);
    console.log(`- Fee: ${feeDecimal} ${usdt.ticker}`);
    ...
    const orderRequest = {
        ...
        to_type: 'ONCHAIN',
        min_onchain_conf: 1,
        dest_rgb_invoice: 'rgb:dqkv_MLl-iiN2WLU-DKAVEkN-tJVc1pR-YCOV184-mQiUCk4', // RGB invoice for payout
