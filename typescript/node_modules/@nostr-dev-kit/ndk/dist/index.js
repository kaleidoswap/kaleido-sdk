...
// src/relay/subscriptions.ts
var NDKGroupedSubscriptions = class extends import_eventemitter3.default {
  subscriptions;
  req;
  debug;
...
var NDKRelay = class extends import_eventemitter32.default {
  url;
  scores;
  connectivity;
  subs;
...
var Zap = class extends import_eventemitter33.default {
  ndk;
  zappedEvent;
  zappedUser;
  constructor(args) {
  ...
  async createZapRequest(amount, comment, extraTags, relays) {
    ...
    const zapRequest = import_nostr_tools4.nip57.makeZapRequest({
...
function generateContentTags(content, tags = []) {
  ...
  content = content.replace(tagRegex, (tag) => {
    try {
      ...
      const { type, data } = import_nostr_tools5.nip19.decode(entity);
      let t;
      switch (type) {
        case "npub":
          t = ["p", data];
...
var NDKKind = /* @__PURE__ */ ((NDKKind2) => {
  ...
  NDKKind2[NDKKind2["DVMJobFeedback"] = 65e3] = "DVMJobFeedback";
  NDKKind2[NDKKind2["DVMJobResult"] = 65001] = "DVMJobResult";
  NDKKind2[NDKKind2["DVMJobRequestTranscription"] = 65002] = "DVMJobRequestTranscription";
  NDKKind2[NDKKind2["DVMNostrContentRecommendation"] = 65008] = "DVMNostrContentRecommendation";
  return NDKKind2;
})(NDKKind || {});
var NDKListKinds = [
  3e4 /* CategorizedPeopleList */,
  30001 /* CategorizedBookmarkList */,
  39802 /* CategorizedHighlightList */,
  30022 /* CategorizedRelayList */,
  30015 /* InterestsList */
];
...
function encode() {
  if (this.isParamReplaceable()) {
    return import_nostr_tools6.nip19.naddrEncode({
      kind: this.kind,
      pubkey: this.pubkey,
    ...
    return import_nostr_tools6.nip19.neventEncode({
      id: this.tagId(),
      relays: [this.relay.url],
      author: this.pubkey
    });
  } else {
    return import_nostr_tools6.nip19.noteEncode(this.tagId());
  }
}
...
// src/events/index.ts
var NDKEvent = class extends import_eventemitter34.default {
  ndk;
  created_at;
  content = "";
  tags = [];
  ...
  async toNostrEvent(pubkey) {
    ...
    try {
      this.id = (0, import_nostr_tools7.getEventHash)(nostrEvent);
    } catch (e) {
    }
    if (this.id)
   ...
   */
  async zap(amount, comment, extraTags, recipient) {
    if (!this.ndk)
      throw new Error("No NDK instance found");
    this.ndk.assertSigner();
    const zap = new Zap({
...
// src/user/follows.ts
async function follows(opts, outbox) {
  if (!this.ndk)
  ...
  if (contactListEvent) {
    ...
    return [...pubkeys].reduce((acc, hexpubkey) => {
...
var NDKUser = class {
  ...
  constructor(opts) {
    ...
    if (opts.hexpubkey)
      this._hexpubkey = opts.hexpubkey;
    if (opts.relayUrls) {
  ...
  get npub() {
    if (!this._npub) {
      ...
      this._npub = import_nostr_tools8.nip19.npubEncode(this.hexpubkey);
    }
    return this._npub;
  }
  set npub(npub) {
  ...
  get hexpubkey() {
    if (!this._hexpubkey) {
      ...
      this._hexpubkey = import_nostr_tools8.nip19.decode(this.npub).data;
    }
    return this._hexpubkey;
  }
  set hexpubkey(hexpubkey) {
  ...
  static async fromNip05(nip05Id) {
    const profile = await import_nostr_tools8.nip05.queryProfile(nip05Id);
    if (profile) {
      return new NDKUser({
  ...
  async validateNip05(nip05Id) {
    if (!this.ndk)
      throw new Error("No NDK instance found");
    const profilePointer = await import_nostr_tools8.nip05.queryProfile(nip05Id);
...
var NDKArticle = class extends NDKEvent {
  constructor(ndk, rawEvent) {
    super(ndk, rawEvent);
...
var NDKHighlight = class extends NDKEvent {
  _article;
  constructor(ndk, rawEvent) {
    super(ndk, rawEvent);
  ...
  async getArticle() {
    ...
    switch (articleTag[0]) {
      case "a":
        ...
        taggedBech32 = import_nostr_tools9.nip19.naddrEncode({ kind: parseInt(kind), pubkey, identifier });
        break;
      case "e":
        taggedBech32 = import_nostr_tools9.nip19.noteEncode(articleTag[1]);
        break;
      case "r":
...
var NDKDVMJobFeedback = class extends NDKEvent {
  constructor(ndk, event) {
    super(ndk, event);
    this.kind ??= 65e3 /* DVMJobFeedback */;
  }
  static from(event) {
...
var NDKDVMJobResult = class extends NDKEvent {
  constructor(ndk, event) {
    super(ndk, event);
    this.kind ??= 65001 /* DVMJobResult */;
  }
  static from(event) {
...
var NDKDVMRequest = class extends NDKEvent {
  constructor(ndk, event) {
    super(ndk, event);
    if (ndk) {
...
var NDKTranscriptionDVM = class extends NDKDVMRequest {
  constructor(ndk, event) {
    super(ndk, event);
    this.kind = 65002 /* DVMJobRequestTranscription */;
  }
  static from(event) {
...
var NDKAppHandlerEvent = class extends NDKEvent {
  profile;
  constructor(ndk, rawEvent) {
    super(ndk, rawEvent);
...
var import_nostr_tools11 = require("nostr-tools");
  ...
  constructor(privateKey) {
    if (privateKey) {
      ...
      this._user = new NDKUser({
        hexpubkey: (0, import_nostr_tools10.getPublicKey)(this.privateKey)
      });
    }
  }
  static generate() {
...
var NDKNip46Backend = class {
  ...
  async handleIncomingEvent(event) {
    ...
    if (!(0, import_nostr_tools11.verifySignature)(event.rawEvent())) {
...
var OutboxTracker = class extends import_eventemitter37.default {
  data;
  ndk;
  debug;
  constructor(ndk) {
...
var NDKPool = class extends import_eventemitter38.default {
...
var NDK = class extends import_eventemitter39.default {
  explicitRelayUrls;
  pool;
  outboxPool;
  signer;
